"""OpenRouter Assistant module for interfacing with OpenRouter API using OpenAI client.

Provides the OpenRouterAssistant class for streaming chat completions and handling assistant responses.
"""
...
class OpenRouterAssistant(Assistant):
    """OpenRouter Assistant for handling chat interactions."""
    ...
    @staticmethod
    def _typed_messages(messages: list[dict[str, str]]) -> typing.Generator[ChatCompletionMessageParam, None, None]:
        for msg in messages:
            yield typing.cast("ChatCompletionMessageParam", msg)

    def run_once(self) -> None:  # noqa: C901, PLR0912
        """Run the OpenRouter Assistant once."""
        ...
        stream: Stream[ChatCompletionChunk] = self._client.chat.completions.create(
            model=MODEL,
            messages=self._typed_messages(self.memory.get_messages()),
        ...
        for part in stream:
            ...
            if delta.role != "assistant":
                continue
            reasoning: str = delta.model_extra.get("reasoning", None)  # pyright: ignore[reportOptionalMemberAccess, reportAssignmentType]
            respons_part: str = delta.content # pyright: ignore[reportAssignmentType]
